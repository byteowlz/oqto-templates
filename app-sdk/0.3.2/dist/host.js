...
import { CONNECT_KIND, isCloseMessage, isProtocolVersion, isReadyMessage, isRecord, isRequestMessage, negotiateProtocol, readSupportedVersions, SUSPEND_KIND, serializeError, supportsV1, supportsV2, } from "./internal/protocol.js";
import { OQTO_APP_PROTOCOL_VERSIONS, } from "./types.js";
/** Create a validated opaque ref in a host adapter without exposing its representation to apps. */
export function createOqtoFileRef(value) {
...
export function createOqtoFileVersion(value) {
 ...
 */
export async function attachOqtoAppFrame(options) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const onAbort = () => finish(() => reject(new OqtoAppError("cancelled", "App frame attachment cancelled")));
        const onMessage = (event) => {
            ...
            if (protocol === undefined) {
                finish(() => reject(new OqtoAppError("unsupported", "App and host share no Oqto App protocol version", {
                    details: { offered: offered.join(",") },
                })));
                return;
            }
            const context = { ...options.adapter.context, protocol };
            ...
            try {
                ...
                channel.port2.close();
                bridge.close();
                finish(() => reject(new OqtoAppError("disconnected", "Could not transfer the App bridge port", { cause: error })));
                return;
            }
            finish(() => resolve(bridge));
        };
        const timer = setTimeout(() => finish(() => reject(new OqtoAppError("timeout", "Timed out waiting for the app frame"))), options.handshakeTimeoutMs ?? 10_000);
...
export function serveOqtoAppPort(adapter, port, limits = {}) {
    ...
    const closeAs = (reason, notify) => {
        if (didClose)
            return;
        didClose = true;
        if (notify)
            port.postMessage({ protocol, kind: "close", reason });
     ...
     */
    const suspendAs = (next) => {
        if (didClose || suspension !== undefined)
            return;
        suspension = next;
        releaseSubscriptions();
        const message = {
    ...
    port.onmessage = (event) => {
        ...
        if (!isRequestMessage(request) || request.protocol !== protocol || didClose)
            return;
        if (suspension !== undefined) {
            sendResult({
                ...
                id: request.id,
                ok: false,
                error: serializeError(new OqtoAppError("suspended", "This App's access was withdrawn", {
        ...
        activeRequests += 1;
        void dispatch(adapter, subscriptions, invocations, port, () => didClose || suspension !== undefined, () => closeAs("transport", false), resolvedLimits, protocol, request.method, request.params)
            .then((value) => {
    ...
    return {
        closed,
        protocol,
        suspend: (next) => suspendAs(next),
        close: () => closeAs("host", true),
    };
}
async function dispatch(adapter, subscriptions, invocations, port, isClosed, onTransportError, limits, protocol, method, params) {
    ...
    switch (method) {
        ...
        case "files.watch.start": {
            ...
            return startSubscription(subscriptions, requireBoundedString(input, "subscriptionId", 256), limits.maxSubscriptions, isClosed, onTransportError, port, protocol, (emit) => files.watch(ref, emit));
        }
        ...
        case "files.watchResources.start": {
            ...
            return startSubscription(subscriptions, requireBoundedString(input, "subscriptionId", 256), limits.maxSubscriptions, isClosed, onTransportError, port, protocol, (emit) => files.watchResources(refs, emit));
        }
        ...
        case "operations.invoke": {
            ...
            try {
                return await operations.invoke(id, rawInput, { signal: controller.signal });
            }
            finally {
        ...
        case "presentation.watch.start": {
            ...
            return startSubscription(subscriptions, requireBoundedString(input, "subscriptionId", 256), limits.maxSubscriptions, isClosed, onTransportError, port, protocol, (emit) => presentation.watch(emit));
        }
        ...
        case "agentContext.watch.start": {
            ...
            return startSubscription(subscriptions, requireBoundedString(input, "subscriptionId", 256), limits.maxSubscriptions, isClosed, onTransportError, port, protocol, (emit) => context.watch(topics, emit, fromRevision === undefined ? {} : { fromRevision }));
        }
        ...
        case "agentContext.action.invoke": {
            ...
            return requireAgentContext(adapter).invokeAction(requireBoundedString(input, "id", 256), parseRevision(input, "expectedContextRevision"), value);
        }
        case "kv.get":
        ...
        case "theme.watch.start": {
            ...
            return startSubscription(subscriptions, requireBoundedString(input, "subscriptionId", 256), limits.maxSubscriptions, isClosed, onTransportError, port, protocol, (emit) => theme.watch(emit));
        }
        case "notifications.notify": {
            const level = requireString(input, "level");
            const message = requireString(input, "message");
            if (message.length > 4096)
                throw new OqtoAppError("too_large", "Notification message is too long");
...
async function startSubscription(subscriptions, subscriptionId, maxSubscriptions, isClosed, onTransportError, port, protocol, subscribe) {
    if (subscriptions.has(subscriptionId))
        throw new OqtoAppError("invalid", "Subscription already exists");
    if (subscriptions.size >= maxSubscriptions) {
        throw new OqtoAppError("quota_exceeded", "App subscription limit reached");
    }
    const pending = { token: Symbol(subscriptionId) };
    subscriptions.set(subscriptionId, pending);
    let unsubscribe;
    try {
        unsubscribe = await subscribe((value) => {
            if (isClosed() || subscriptions.get(subscriptionId)?.token !== pending.token)
                return;
            const event = {
                protocol,
                kind: "event",
        ...
        if (subscriptions.get(subscriptionId) === pending)
    ...
    if (isClosed() || subscriptions.get(subscriptionId) !== pending) {
        unsubscribe();
        if (isClosed())
            throw new OqtoAppError("disconnected", "App bridge closed while subscribing");
        return;
    }
    subscriptions.set(subscriptionId, { token: pending.token, unsubscribe });
}
function stopSubscription(subscriptions, subscriptionId) {
    const subscription = subscriptions.get(subscriptionId);
    if (subscription === undefined)
        throw new OqtoAppError("gone", "Subscription does not exist");
    subscriptions.delete(subscriptionId);
    if ("unsubscribe" in subscription)
        subscription.unsubscribe();
}
...
function requireFiles(adapter) {
    requireCapability(adapter, "files");
    if (!adapter.files)
        throw new OqtoAppError("denied", "Files capability is not granted");
    return adapter.files;
}
function requireKv(adapter) {
    requireCapability(adapter, "kv");
    if (!adapter.kv)
        throw new OqtoAppError("denied", "KV capability is not granted");
    return adapter.kv;
}
function requireTheme(adapter) {
    requireCapability(adapter, "theme");
    if (!adapter.theme)
        throw new OqtoAppError("denied", "Theme capability is not granted");
    return adapter.theme;
}
function requireNotifications(adapter) {
    requireCapability(adapter, "notifications");
    if (!adapter.notifications)
        throw new OqtoAppError("denied", "Notifications capability is not granted");
    return adapter.notifications;
}
function requireOperations(adapter) {
    requireCapability(adapter, "operations");
    if (!adapter.operations)
        throw new OqtoAppError("denied", "Operations capability is not granted");
    return adapter.operations;
}
function requirePresentation(adapter) {
    requireCapability(adapter, "presentation");
    if (!adapter.presentation)
        throw new OqtoAppError("denied", "Presentation capability is not granted");
    return adapter.presentation;
}
function requireAgentContext(adapter) {
    requireCapability(adapter, "agent_context");
    if (!adapter.agent_context)
        throw new OqtoAppError("denied", "Agent Context capability is not granted");
    return adapter.agent_context;
}
function resolveHostVersions(explicit) {
...
function parseRefs(input) {
    ...
    return refs.map((ref) => {
        if (typeof ref !== "string")
            throw new OqtoAppError("invalid", "refs must contain strings");
...
function requireRecord(value) {
    if (!isRecord(value))
        throw new OqtoAppError("invalid", "Operation parameters must be an object");
    return value;
}
...
function parseTopics(input) {
    ...
    if (new Set(values).size !== values.length)
        throw new OqtoAppError("invalid", "topics must be unique");
    return values;
}
function parseRevision(input, key) {
    const value = input[key];
...
function parseOptionalRevision(input, key) {
    return input[key] === undefined ? undefined : parseRevision(input, key);
}
function parsePickOptions(input) {
    const result = {};
...
function validateLimit(name, value, fallback) {
    if (value === undefined)
        return fallback;
    if (!Number.isSafeInteger(value) || value <= 0) {
